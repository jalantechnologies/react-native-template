...
1. Update `app.json` `name` and `displayName`.
2. Update `package.json` `name`.

## Android

- `android/settings.gradle`: set `rootProject.name`.
...
- `android/app/src/main/res/values/strings.xml`: adjust `app_name`.

## iOS

- Rename `ios/Boilerplate` folder to your app name.
- Update targets in `ios/Podfile`.
- Update `CFBundleDisplayName` in `ios/Boilerplate/Info.plist`.
- Run `yarn pod-install` after changes.

## CI/CD Metadata

- `.github/workflows/*.yml`: update any hardcoded project identifiers.
- `sonar-project.properties`: set the correct `sonar.projectKey`.
